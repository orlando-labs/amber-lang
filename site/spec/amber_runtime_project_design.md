# Amber — runtime и проектный слой компилируемой реализации

**Назначение документа:** инженерная спецификация reference implementation Amber:
compiler pipeline, bytecode ISA, runtime ABI, объектная модель,
lifetime/GC/pinning/FFI, формат `.amberbc`, loader/verifier, minimal MOP, а также
implementation matrix, milestone gates и backlog.

Языковая спецификация вынесена в
[`amber_unified_final_spec.md`](amber_unified_final_spec.md). Этот документ описывает,
*как* язык реализуется, и не переопределяет наблюдаемую семантику языка.
