...
- `profile/modern`, `.amberbc` `AGNT`/`CNTR`/`PRIV`/`WFLW`, and `runtime/vm`
  ...
  and runtime mirror/validation exposure;
- `runtime/vm` W8.3/W8.5 collections contract with closure-block execution for eager sequence `each/map/filter_map/flat_map/select/reject/reduce/find/any?/all?/none?/first/count/group_by/to_a/lazy`, mutating `Array` / `Set` `filter_map!`, deterministic `EmptyCollectionError` for empty `reduce` without init, ordered `Map#keys/#values/#entries/#map/#filter_map/#select/#reject/#transform_values/#each`, and v20.6 value-keyed `Map` / `Set` semantics;
