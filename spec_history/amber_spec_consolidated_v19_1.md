> Редакция v19.1: поверх v19 в эту версию дополнительно включены handoff-sync правки для передачи в реализацию: выровнены parser/AST/HIR contracts (`unless`, `mixin`-expr, `extend`, `class_method def`, `TypeTerm`), заморожен canonical physical encoding `.amberbc`, нормализована runtime error taxonomy, дожат sync/runtime contract (`Channel.close`, closed-channel semantics, non-reentrant `Mutex`, seq-cst `Atomic`) и добавлены machine-readable registries/engineering notes для безопасного распараллеливания команды. Языковые решения v16-v19 не расширяются; документ доводит инженерный пакет до truly handoff-ready состояния.
...
```text
/spec/
  amber_spec_consolidated_v19_1.md
  changelog/
  registries/
    tokens.yaml
    diagnostics.yaml
...
- `spec/registries/` обязан содержать machine-readable registries для token kinds, diagnostics, opcodes, bytecode sections и runtime error taxonomy;
