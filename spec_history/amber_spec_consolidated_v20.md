> Редакция v20.0: поверх v19.2 добавлен слой **Modern Pressure Profiles** — optional profile-level контракты для capability/sandbox execution, effect rows, observability/replay, DataFrame/columnar BI, schema/API contracts, Wasm Component host plugins, accelerator kernels, AI-agent tooling/provenance, contracts/property testing, privacy/taint/lineage и durable workflows. Core syntax, v1 runtime semantics, `.amberbc` physical encoding, frozen-world и production dynamic profile не ломаются: v20 оформляет современные вызовы как opt-in профили, HIR/bytecode/tooling metadata и conformance lanes.
