> Редакция v9: в эту версию включены нормативные решения по reference bytecode VM, runtime ABI, explicit lifetime control (`destroy!` + `memory.dealloc`), reference collector/pinning/FFI profile и стабильному compiled-module/loader profile.
...
- `(` `)` `[` `]` `{` `}` `,`;
- ключевые слова `not`, `and`, `or`, `in`, `if`, `else`, `case`, `when`, `def`, `class`, `while`, `until`, `do`, `loop`;
...
- matcher expression в `def`-клаузаx, block params и pattern assignment.
...
### 17.8. Matcher expressions только в `case`

В `case` допускается fallback-форма:
...
Во всех остальных pattern-контекстах это запрещено.
...
- `case` со структурными паттернами и matcher expressions;
...
- `===` / `deconstruct*` с неправильным типом возвращаемого значения -> `TypeError`.
...
AstLoop(body)
AstBreak(value?)
AstCase(scrutinee, arms[], else_body?)
...
`case` lower'ится в `HMatchDispatch`, который использует тот же pattern-engine, что и `HMethod.clauses`.
...
Ниже — только реальные незакрытые зоны, которые остались **после закрытия parser core P0** в этой редакции.
...
## Q2. Финальный модульный / импортный синтаксис
...
## Q6. Динамические pattern-objects
...
## Q7. Формальный статус `matcher expressions` вне `case`
...
## Q9. Нужна ли строгая match-форма поверх безопасного `case`
...
- source-level package/import syntax и package manager policy;
...
| G11. Modules & metaprogramming | Намерение без нормализации | Выбрать import model и минимальный MOP: open classes, `define_method`, `send`, `method_missing` | G1, G5, G9 | Можно описать загрузку кода и позднее расширение классов без двусмысленности |
...
- matcher expressions только в `case`, а не везде;
- поздний commit auto-assign после dispatch;
- `case` и patterns через ограниченный protocol set.
...
Алгоритм GC остаётся открытым вопросом, но есть две уже фиксированные нормы:
...
3. guard компилируется только после успешного pattern region;
4. первая успешная clause прыгает в свой body;
5. иначе — `else_body` или fail mode.
...
Эта часть закрывает вопрос compiled-module distribution profile для Amber v1. Она **не** выбирает окончательный surface-синтаксис `import`/`require`/`from ... import ...`; этот вопрос по-прежнему остаётся выше, на уровне исходного языка и package system. Но она фиксирует, как выглядит уже скомпилированный модуль и как он загружается runtime'ом.

Нормативный вывод:

- stable compiled artifact Amber v1 называется `.amberbc`;
- `.amberbc` описывает `BcModule` в сериализованном виде;
- loader/verifier обязаны работать с `.amberbc` независимо от source-level spelling импортов;
...
Source-level spelling импорта остаётся открытым, но в `.amberbc` зависимости всегда нормализуются до logical module ids.
