> Редакция v16: поверх v15 в эту версию дополнительно включены closure-профили второй волны: optional **Amber/Typed** static profile, package/distribution/signing/hot-reload policy, расширенный read-only reflection/introspection API, class-side mixins/`extend`, advanced concurrency extensions (`move`, `select`, supervisor policies, async I/O awaitables), memory/lifetime second wave без field modifiers и canonical MIR/SSA + native/JIT/AOT + frozen-image profile. После этой редакции в документе не остаётся незакрытых spec-level вопросов: дальше остаются только implementation backlog и toolchain work-items.
