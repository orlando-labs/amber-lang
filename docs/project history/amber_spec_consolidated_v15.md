> Редакция v15: поверх v14 в эту версию дополнительно включены reference implementation blueprint P0/P1, канонические dump-контракты для AST/HIR/diagnostics, обязательный каталог diagnostic codes v1, layout golden/corpus-файлов и bootstrap order для reference toolchain. Ранее зафиксированные implementation gate для reference-имплементации, minimal type envelope (`-> TypeTerm`, минимальная grammar `TypeTerm` и runtime type-hooks), обязательный stdlib contract для chainable collections, формальная матрица диагностик компилятора, policy по underscore-спецформам, окончательная граница bare matcher expressions и v1-решение по field lifetime annotations, а также source-level module syntax, reference bytecode VM, runtime ABI, explicit lifetime control (`destroy!` + `memory.dealloc`), reference collector/pinning/FFI profile, compiled-module/loader profile, dynamic pattern objects в explicit-binding profile, строгая match-форма `case!`, minimal MOP / reflective dispatch, mixin/`include` profile и Amber/Frozen profile сохраняются без регрессий.
...
Amber в текущем виде — **консолидированный draft+ с закрытым implementation gate и зафиксированным reference blueprint для runtime P0/P1**. Ядро синтаксиса и семантики уже достаточно устойчиво, чтобы писать не только прототип парсера и интерпретатора, но и первый байткодный runtime, а инженерная граница старта теперь дополнительно поддержана каноническими AST/HIR/diagnostic dump-контрактами, обязательным каталогом diagnostic codes v1, layout-моделью corpus/golden-файлов и bootstrap order для reference toolchain. Ранее зафиксированные minimal type envelope для parser/HIR/runtime hooks, обязательный stdlib contract для chainable collections, формальная матрица диагностик, policy по underscore-спецформам, окончательная граница bare matcher expressions и v1-решение по field lifetime annotations, а также reference execution profile, reference lifetime profile, collector/pinning/FFI profile, compiled-module/loader profile, strict `case!`, source-level modules, minimal MOP, mixin/`include` profile и frozen-world boundary сохраняются без регрессий: register/slot bytecode VM, frame/closure/object ABI, pattern-matching opcode family, ownership model без GIL, explicit destruction/deallocation (`destroy!` + `memory.dealloc`), non-moving generational collector, pinning boundary для native interop, `.amberbc`-формат и verifier/loader state machine, static `package/import/export`, reopenable named classes, named mixins, declarative `include`, linearized ancestor composition, reflective `define_method`, builtin `send(...)`, `method_missing` fallback и двухфазная модель dispatch-world (`open` -> `frozen`). Полностью незавершёнными остаются уже не блокеры старта reference implementation, а вторая волна дизайна и toolchain: full static checker/inference, package-manager/distribution policy, extended reflection/introspection API, class-side mixins/`extend`, advanced concurrency extensions и backend/JIT/AOT toolchain поверх уже зафиксированной frozen-boundary.
...
Открытыми остаются уже не syntax-level, а ecosystem/toolchain-level вопросы: package manager, trust/signing и hot reload.
...
## Q4. Типовая система: grammar, semantics, inference [частично закрыто в v14]

В v14 закрыт minimal type envelope для implementation gate:

- `-> TypeTerm` как return type syntax;
- минимальная grammar `TypeTerm`;
- record/map types с ключами и `**rest-type`;
- runtime type-hooks на parameter bind, `expr as TypeTerm` и return boundary.

Открытыми остаются уже вопросы второй волны:

- full static checker и inference;
...
## Q14. Нужны ли полевые lifetime-аннотации (`owned`, `weak`, `borrowed`) [закрыто для v1 в v14]

В v14 фиксируется следующее решение:

- `owned`, `weak`, `borrowed` не входят в source grammar v1;
- lifecycle v1 по-прежнему строится вокруг пользовательского `destroy!` и `memory.dealloc`;
- вопрос полевых lifetime-аннотаций переносится во вторую волну и не блокирует reference runtime.

То есть language core v1 закрывается без field-level lifetime annotations.
...
11. только после этого — full checker, extended stdlib, MOP second wave и frozen/AOT profile.
...
| Трек | Состояние | Ближайший шаг | Зависимости | Критерий выхода |
|---|---|---|---|---|
| G10. Типовая система v1 | Minimal envelope закрыт; full checker deferred | Реализовать parser/runtime hooks для `TypeTerm`, return types и record/map forms, затем отдельно поднять checker/inference | G1–G9 | TypeTerm не конфликтует с pattern grammar, а checker-профиль можно развивать независимо от старта VM |
| G11. Modules & metaprogramming | Mostly closed: source-level modules, minimal MOP, mixin/`include` profile и frozen-boundary зафиксированы; distribution open | Реализовать open-class/open-mixin transactions, `define_method`, builtin `send`, `method_missing`, `include` linearization, world freeze state и затем отдельно решить distribution layout | G1, G5, G6e, G9 | Dispatch-world и reflective slow-path реализуются без двусмысленности; distribution/toolchain остаются отдельной второй волной |
...
Неблокирующими вопросами второй волны остаются:

- full static checker / inference / variance / type-narrowing;
- package manager / registry / signing / hot reload;
- extended reflection / introspection API;
- class-side mixins / `extend`;
- advanced concurrency extensions beyond current no-GIL core;
- field lifetime annotations beyond v1;
- MIR/SSA, JIT, native AOT и frozen-image deployment.
