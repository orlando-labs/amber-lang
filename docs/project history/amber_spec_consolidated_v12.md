> Редакция v12: в эту версию дополнительно включены нормативные решения по minimal MOP / reflective dispatch и frozen-world boundary: reopenable named classes, builtin `define_method`, builtin `send(...)`, `method_missing` fallback и Amber/Frozen profile. Ранее зафиксированные source-level module syntax, reference bytecode VM, runtime ABI, explicit lifetime control (`destroy!` + `memory.dealloc`), reference collector/pinning/FFI profile, compiled-module/loader profile, dynamic pattern objects в explicit-binding profile и строгая match-форма `case!` сохраняются без регрессий.
...
Amber в текущем виде — **консолидированный draft+**. Ядро синтаксиса и семантики уже достаточно устойчиво, чтобы писать не только прототип парсера и интерпретатора, но и первый байткодный runtime. В этой редакции дополнительно закрыты reference execution profile, reference lifetime profile, reference collector/pinning/FFI profile, compiled-module/loader profile, dynamic pattern objects в explicit-binding profile, строгая match-форма `case!`, source-level module syntax, minimal MOP / reflective dispatch profile и frozen-world boundary для AOT: register/slot bytecode VM, frame/closure/object ABI, pattern-matching opcode family, ownership model без GIL, explicit destruction/deallocation (`destroy!` + `memory.dealloc`), non-moving generational collector, pinning boundary для native interop, `.amberbc`-формат и verifier/loader state machine, явные dynamic matcher bindings без скрытой инъекции локалов, strict fail-mode для `case!`, статический пакетно-импортный профиль `package/import/export`, reopenable named classes, reflective `define_method`, builtin `send(...)`, `method_missing` fallback и двухфазная модель dispatch-world (`open` -> `frozen`). Полностью незавершёнными остаются прежде всего типовая система, mixin/`include`/namespace-composition слой, package-manager/distribution policy, extended reflection/introspection API, field-level lifetime annotations (`weak`/`borrowed`) и собственно backend/JIT/AOT toolchain поверх уже зафиксированной frozen-boundary.
...
Amber v12 фиксирует **minimal open-class model**.
...
### 8.12. Reflective `define_method`

Минимальный reflective API v12 задаётся builtin-функцией:
...
- первый аргумент обязан быть class object;
...
- в v1 `define_method` определяет или заменяет **instance method**;
...
- class-side reflective `define_method`, remove/alias/visibility hooks и mixin-операции в минимальный v1-профиль не входят.

Успешный `define_method` считается world mutation и обязан обновлять dispatch invalidation metadata по правилам части X.

### 8.13. Reflective `send`
...
`method_missing` получает в v12 минимальную, но нормативную семантику.
...
### 8.15. World mutation и frozen-world boundary

Amber v12 разводит **data mutation** и **world mutation**.

К world mutation относятся:

- создание нового named class object;
- reopen существующего класса;
- `define_method`;
...
### 8.16. Что не входит в minimal MOP v12

Сознательно вне минимального v1-профиля остаются:

- `include` / mixins / namespace composition;
...
Выбрасывается, когда код в frozen-profile пытается выполнить world mutation после freeze barrier: reopen класса, `define_method` или позднюю загрузку Amber-модуля в уже frozen dispatch-world.
...
- MOP-level `include` / mixins / namespace composition;
...
- open class через повторную `class Name: ...`-форму;
- `define_method(Target, :name) |...|: ...`;
...
- ссылка из `pattern(expr)` на имя, вводимое тем же enclosing pattern;
- `define_method` с не-class target;
...
## Q3. Глубина нормализации метапрограммирования [закрыто в v12]

В v12 зафиксирован **minimal MOP profile**:
...
- после freeze любая world mutation (`class` reopen, `define_method`, поздняя Amber module load в тот же world) запрещена и даёт `WorldFrozenError`;
...
Открытым остаётся уже не language boundary, а backend/toolchain-уровень: SSA/MIR, optimizer, codegen, packaging и deployment frozen-image'ов.
...
| G11. Modules & metaprogramming | Mostly closed: source-level modules, minimal MOP и frozen-boundary зафиксированы; mixins/distribution open | Реализовать open-class transactions, `define_method`, builtin `send`, `method_missing`, world freeze state и затем отдельно решить `include`/package layout | G1, G5, G6e, G9 | Dispatch-world и reflective slow-path реализуются без двусмысленности; mixin/distribution остаются отдельной второй волной |
...
5. реализовать open-class transaction path, `define_method`, `method_missing` fallback и `WorldFrozenError` guards поверх уже фиксированного ISA;
...
- minimal MOP и frozen-world boundary уже нормализованы, но mixin/introspection слой ещё открыт;
...
- invalidation model для `open class`, `define_method`, `send`, `method_missing` задан;
...
Эта часть закрывает для Amber v12 два старых узла:

1. **минимальный MOP / reflective dispatch profile**;
2. **границу между dynamic Amber и Amber/Frozen для AOT/JIT**.
...
Все более широкие темы — mixins, `include`, hot reload, расширенная рефлексия, method alias/removal, package distribution policy — сознательно остаются за пределами этой части.

## 2. Объём минимального MOP v12
...
В минимальном v12 class-body нормализуется как **declarative body**. Для этого профиля нормативно существенны:
...
- dynamic clause accretion across reopen boundaries в v12 отсутствует.

## 4. Reflective `define_method`
...
Минимальный контракт v12:

- первый аргумент обязан быть class object;
- второй аргумент обязан быть `Symbol` или `Str`;
- реализация метода задаётся либо block suffix, либо явным callable-аргументом, но не обоими одновременно;
- в v12 `define_method` воздействует только на **instance-side** target-класса.

### 4.3. Implementation source
...
## 7. World mutation model

Amber v12 различает обычную мутацию данных и мутацию самого dispatch-world.

### 7.1. Что считается world mutation
...
- будущие ancestor/mixin-операции, когда они появятся;
- загрузка Amber-модуля в уже frozen dispatch-world.

### 7.2. Что не считается world mutation
...
Для source-level `class`-reopen transaction весь body считается одной publish-point transaction. Для `define_method` transaction — это один вызов.

## 8. Frozen-world profile
...
### 8.4. Post-freeze behavior

После freeze:

- world mutations запрещены;
- попытка reopen класса, `define_method` или поздней Amber module load в тот же world обязана завершаться `WorldFrozenError` либо быть отклонена ещё до выполнения;
...
### 9.3. No dedicated opcode for world mutation

`class`-reopen и `define_method` **не обязаны** иметь специальные ISA-opcodes.
...
## 10. Что эта часть закрывает, а что оставляет открытым
...
## 11. Практический следующий шаг после этой части
...
2. ввести `HSendDyn` и `SEND_DYN`;
3. реализовать world-state (`open`/`frozen`), `world_epoch` и version invalidation;
4. закрыть open-class transaction path и `method_missing` fallback в VM/loader tests;
5. после этого переходить к реальному bytecode runtime и frozen-loader execution tests.
